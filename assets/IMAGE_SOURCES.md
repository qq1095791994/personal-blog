...
首屏实时 3D 古街由 `assets/world/` 中的网页代码渲染。它以本人《UE 仙侠》的真实作品截图为参照，重新制作近似的街道、建筑和物件，并提供 360° 环绕、街道漫游、视角切换及原作对照。作品资料没有原始 UE 场景模型；这个网页场景不是原项目模型、自动扫描结果或 AI 精确重建，截图未展示的立面和空间属于创作性补全。WebGL 不可用时显示《UE 仙侠》原作截图 `work/fantasy-04.webp`。
...
首屏主要参照的原图与网页文件对应如下，均为原作截图压缩后的 WebP，并非 AI 生成：

| 网页图片 | 作品资料原图 | 用途 |
| --- | --- | --- |
| `work/fantasy-04.webp` | `游戏类型/UE仙侠 (4).png` | 正街、入口牌坊、两侧建筑和灯笼的视觉参照及首屏后备图 |
| `work/fantasy-05.webp` | `游戏类型/UE仙侠 (5).png` | 侧巷、木楼、红柱及街边物件参照 |
| `work/fantasy-06.webp` | `游戏类型/UE仙侠 (6).png` | 鸟瞰道路、屋顶、花坛与摊位的布局参照 |

`auto-motion.mp4` 与 `motion-loop.mp4` 来自同一作品目录中的短片。

## AI 概念视觉

以下两张图为 AI 生成，仅用于表达智能空间方向，不代表真实客户或已交付项目。`concept-digital-twin.webp` 在文章中明确标注；`concept-intelligent-space.webp` 目前保留为素材档案，已不用于首屏。
